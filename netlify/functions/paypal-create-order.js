...
exports.handler = async (event) => {
  try {
    ...
    // ====================== CUSTOM_ID : variant_id + COLOR (pour Order History) ======================
    const custom_id = cart
      .map(item => `${item.cj_variant_id || ''}|${item.color || 'Standard'}`)
      .join('||');
    ...
    // payer prefill (inchangé)
    let payer = {};
    if (shipping.email) payer.email_address = shipping.email;
    if (shipping.firstName || shipping.lastName) payer.name = { given_name: shipping.firstName || '', surname: shipping.lastName || '' };
