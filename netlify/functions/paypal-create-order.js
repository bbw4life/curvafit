...
function computeTotals(cart, settings) {
  ...
  // Cart subtotal (paid items only — free promo items have price 0 already)
  ...
  return {
    ...
    finalTotal:       subtotal + effectiveShipping + effectiveTax,
  };
}

// ── Enforce free promo items (price = 0) from settings ───────────────
// This ensures even if the client tampers prices, promo items are always $0
function sanitizeCart(cart, settings) {
  ...
  return cart.map(item => {
    if (item.isFreePromo) {
      // Only honour free promo if paid qty actually qualifies
      if (paidQty >= buyQty) {
        return { ...item, price: 0 };
      } else {
        // Remove freePromo flag — charge normally
...
exports.handler = async (event) => {
  try {
    ...
    const { subtotal, shippingCost, taxAmount, finalTotal } = computeTotals(cart, settings);
