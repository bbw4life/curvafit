...
function parseProductsData(rawData) {
  ...
  const featured = activeProducts.slice(0, 4).map((p, index) => {
    ...
    return {
      ...
      colors:      (p.colors || []).filter(c => c.active !== false),
      sizes:       p.sizes || [],
    };
  });

  // Promos from settings
  const rawPromos = settings.promos || [];
  const promos = rawPromos.length > 0
    ? rawPromos.map(p => ({ code: p.code, label: `${p.percent}% off — ${p.items} items or more` }))
    : [
...
// ── Product card — all data comes from products.data.json ─────────────────────
function cProductCard(product, promoCode) {
  ...
  // Color swatches (max 4) from real data
  const swatchesHTML = (product.colors || []).slice(0, 4).map(c =>
    `<span style="display:inline-block;width:14px;height:14px;border-radius:50%;background:${c.hex || '#ccc'};border:1.5px solid #fff;box-shadow:0 0 0 1px #d1d5db;margin-right:3px;" title="${c.name}"></span>`
  ).join('');

  const colorLineHTML = swatchesHTML
    ? `<p style="margin:0 0 7px;">${swatchesHTML}</p>`
    : '';

  // Sizes from real data
  const sizesHTML = (product.sizes || []).length > 0
    ? `<p style="margin:0 0 7px;font-family:'DM Sans',Arial,sans-serif;font-size:11px;color:#9ca3af;">Sizes: ${product.sizes.join(' · ')}</p>`
    : '';
...
<table width="100%" cellpadding="0" cellspacing="0" role="presentation"
  ...
  <tr>
    ...
    <td style="padding:14px 16px;vertical-align:middle;">
      <span style="display:inline-block;padding:2px 10px;border-radius:20px;background:linear-gradient(90deg,#7c3aed,#db2777);font-family:'DM Sans',Arial,sans-serif;font-size:10px;font-weight:700;color:#fff;letter-spacing:0.08em;text-transform:uppercase;margin-bottom:6px;">${product.badge}</span>
      <p style="margin:0 0 5px;font-family:'Cinzel',Georgia,serif;font-size:13px;font-weight:700;color:#1e1b4b;line-height:1.3;">${product.title}</p>
      <p style="margin:0 0 5px;">
        <span style="font-family:'DM Sans',Arial,sans-serif;font-size:16px;font-weight:800;color:#7c3aed;">${product.price}</span>
        <span style="font-family:'DM Sans',Arial,sans-serif;font-size:12px;color:#9ca3af;text-decoration:line-through;margin-left:6px;">${product.oldPrice}</span>
      ...
      ${colorLineHTML}
      ${sizesHTML}
      ${promoCode ? `<p style="margin:0 0 8px;display:inline-block;padding:3px 9px;border-radius:8px;background:#fdf4ff;border:1.5px dashed #a855f7;font-family:'DM Sans',Arial,sans-serif;font-size:11px;color:#6b21a8;">Code: <strong>${promoCode}</strong></p>` : ''}
      <a href="${product.url}" target="_blank"
