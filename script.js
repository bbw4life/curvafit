document.addEventListener('DOMContentLoaded', () => {
  ...
  // ====================== FETCH PRODUCTS ======================
  // Charge TOUS les produits du JSON y compris les nouveaux :
  ...
  fetch('/products.data.json')
    ...
    .then(data => {
      ...
      // ====================== PRODUCT CARDS ======================
      // Injecte automatiquement dans grid-1, grid-2, grid-3 ET grid-4
      // pour ankle-weights, cooling-towel, massage-ball, gym-bag
      ...
      if (productSection) {
        ...
        if (enableMediaZoom) {
          ...
          let translateX = 0;
          let translateY = 0;
          let isDraggingZoom = false; // FIX: renommé pour éviter conflit
          let lastTouchX = 0;
          let lastTouchY = 0;
          let maxTranslateX = 0;
          let maxTranslateY = 0;
          ...
          function calculateBounds() {
            ...
            maxTranslateX = Math.max(0, (dispW * scale - contW) / 2);
            maxTranslateY = Math.max(0, (dispH * scale - contH) / 2);
          }
        ...
        // Delivery dates — FIX: renommé showTextDelivery pour éviter conflit avec showText
        if (prod) {
      ...
      if (bundleContainer) {
        ...
        if (product) {
          ...
          function populateSelectors(container) {
            ...
            previewImgs.forEach(img => { img.src = product.image; });
          }
          ...
          function getSelectedValues(selectorsContainer) {
            ...
            selects.forEach(select => {
              ...
              if (value !== "") {
                if (label.includes("color")) values.color = value;
                else if (label.includes("size")) values.size = value;
              }
            });
            return values;
          }
            ...
            option.addEventListener("click", function(e) {
              ...
              if (!wasChecked) {
                ...
                if (selection) {
                  ...
                  calculateBundlePrice(this.dataset.bundle);
                }
              }
            });
          });
          ...
          document.querySelectorAll(".bundle-add-btn").forEach(btn => {
            btn.addEventListener("click", function() {
              ...
              let discount = 0;
              if (type === "single") discount = (product.single_discount || 0) / 100;
              else if (type === "duo") discount = (product.duo_discount || 0) / 100;
              else if (type === "trio") discount = (product.trio_discount || 0) / 100;
              const ratio = product.compare_price / product.price;
              if (type === "single") {
                if (!hasColors && !hasSizes) {
                  const varPrice = product.price;
                  const variant = product.variants ? product.variants[0] : null;
                  items.push({ id: product.id, title: product.title, price: varPrice * (1 - discount), compare_price: varPrice * ratio, image: itemImage, size: null, color: null, quantity: 1, fromBundle: true, cj_product_id: product.cj_id, cj_variant_id: variant ? variant.vid : null });
                } else {
  ...
  document.querySelectorAll('section').forEach(sec => {
    if (!sec.hasAttribute('data-scroll-reveal')) sec.setAttribute('data-scroll-reveal', '');
  });
  ...
  const revealOnScroll = () => {
    ...
    revealElements.forEach(el => {
      if (el.getBoundingClientRect().top < windowHeight - 100) el.classList.add('revealed');
    });
  };
  ...
  counters.forEach(counter => {
    ...
    const observer = new IntersectionObserver(entries => { if (entries[0].isIntersecting) updateCount(); });
  ...
  if (carousel) {
    ...
    let carouselIndex = 0; // FIX: renommé pour éviter conflit
  ...
  if (audioPlayer) {
    let isDraggingAudio = false; // FIX: renommé pour éviter conflit
    let offsetAudioX, offsetAudioY;
    const startDrag = (e) => {
  ...
  productCards.forEach(card => { card.addEventListener('mouseenter', () => { card.style.transition = 'transform 0.3s ease'; }); });
  ...
  function aggregateData(tab) {
    ...
    const data = labels.map(key => aggregated[key].reduce((sum, val) => sum + val, 0) / aggregated[key].length);
    return { labels, data, description: `Your ${tab} progress based on your entered data. Keep adding to track accurately!` };
  }
  ...
  function updateChart(tab) {
    ...
    const ctxChart = document.getElementById('progress-chart')?.getContext('2d'); // FIX: renommé ctxChart
    if (!ctxChart) return;
    chartInstance = new Chart(ctxChart, {
      type: 'line',
      data: { labels, datasets: [{ label: 'Progress (e.g., Weight or Points)', data, borderColor: '#e91e63', backgroundColor: 'rgba(233, 30, 99, 0.2)', fill: true, tension: 0.4 }] },
  ...
  accordionHeaders.forEach(header => { header.addEventListener('click', () => { header.parentElement.classList.toggle('active'); }); });
  ...
  // ====================== PROGRESS CURVE ======================
  // FIX: renommé ctxCurve pour éviter conflit avec ctx du progress-chart
...
<button class="remove-item"><i class="fi fi-sr-trash"></i></button>`;
  ...
  function updateSubtotal() {
    if (subtotalElement) subtotalElement.textContent = `Subtotal: $${cart.reduce((sum, item) => sum + (item.price * item.quantity), 0).toFixed(2)}`;
  }
  ...
  function addToCart(e) {
    ...
    else cart.push({ id: product.id, title: product.title, price: varPrice, compare_price: varCompare, image: itemImage, size: selectedSize, color: selectedColor, quantity: quantity, cj_product_id: product.cj_id, cj_variant_id: cjVariantId });
  ...
  function openCartDrawer() { renderCart(); cartDrawer.classList.add('active'); overlay.classList.add('active'); }
  ...
  // FIX: null checks ajoutés pour éviter crash sur pages sans ces éléments
  ...
  if (paulContainer) {
    ...
    if (paulSlides.length > 1) {
      let paulCurrentSlide = 0; // FIX: renommé pour éviter conflit
      let paulSlideTimer;
      ...
      paulIndicators.forEach((ind, i) => {
        ind.addEventListener('click', () => { clearInterval(paulSlideTimer); paulShowSlide(i); paulSlideTimer = setInterval(paulNextSlide, 5000); });
      });
  ...
  if (francenelContainer) {
    ...
    if (francSlides.length > 1) {
      let francCurrentSlide = 0; // FIX: renommé pour éviter conflit
      let francSlideTimer;
      ...
      francIndicators.forEach((ind, i) => {
        ind.addEventListener('click', () => { clearInterval(francSlideTimer); francShowSlide(i); francSlideTimer = setInterval(francNextSlide, 5000); });
      });
  ...
  // ================================================================
  //   SHOP HERO BANNER — Slideshow + Thumbnails
  // ================================================================
  (function() {
    const heroSlides = [
    ...
    function placeThumbs() {
      ...
      if (isMobile && bannerContent && !bannerContent.contains(thumbsWrap)) {
        bannerContent.insertBefore(thumbsWrap, bannerContent.firstChild);
      } else if (!isMobile && bannerContent && bannerContent.contains(thumbsWrap)) {
        hero.appendChild(thumbsWrap);
      } else if (!isMobile && !hero.contains(thumbsWrap)) {
        hero.appendChild(thumbsWrap);
      }
    }
    ...
    let heroCurrentSlide = 0; // FIX: renommé pour éviter conflit avec announcementCurrent
    let heroTimer;

    function heroGoTo(index) {
      const allSlides = hero.querySelectorAll('.hero-slide');
      const allThumbs = thumbsWrap.querySelectorAll('.hero-thumb');
      allSlides[heroCurrentSlide].classList.remove('active');
      allThumbs[heroCurrentSlide].classList.remove('active');
      heroCurrentSlide = index;
      allSlides[heroCurrentSlide].classList.add('active');
      allThumbs[heroCurrentSlide].classList.add('active');
  ...
  // ====================== ANNOUNCEMENT BAR ======================
  // FIX: renommé announcementCurrent pour éviter conflit avec heroCurrentSlide et autres
  ...
  function nextAnnouncementItem() {
    showAnnouncementItem((announcementCurrent + 1) % announcementItems.length);
  }
  if (announcementItems.length > 0) {
    setInterval(nextAnnouncementItem, 4000);
  }
...
document.addEventListener('click', function(e) {
  if (e.target.closest('.swatch')) {
    ...
    if (isMobile) {
      ...
      if (mediaSlider) {
        setTimeout(() => { mediaSlider.scrollIntoView({ behavior: 'smooth', block: 'start' }); }, 50);
      }
    }
  }
});
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (paulPopupOverlay) {
    paulPopupOverlay.addEventListener('click', (e) => {
      if (e.target === paulPopupOverlay && !isAccountPage) closePaulPopup();
    });
  }
  ...
  document.querySelectorAll('.password-toggle').forEach(toggle => {
    toggle.addEventListener('click', function () {
      ...
      if (input.type === 'password') { input.type = 'text'; icon.classList.remove('fi-sr-eye'); icon.classList.add('fi-sr-eye-crossed'); }
      else { input.type = 'password'; icon.classList.remove('fi-sr-eye-crossed'); icon.classList.add('fi-sr-eye'); }
    });
  });
  ...
  window.openSavedItems = () => {
    if (localStorage.getItem('isLoggedIn') !== 'true') { window.showToast("Connectez-vous pour voir vos articles sauvegardés"); return; }
  ...
  async function loadAccountStats() {
    ...
    try {
      ...
      if (!historyContainer) { console.warn("⚠️ .order-history non trouvé dans le DOM"); return; }
      if (data.history && Array.isArray(data.history) && data.history.length > 0) {
        ...
        setTimeout(() => {
          ...
          document.addEventListener('click', function(e) {
            ...
            if (url === 'shop.html') { console.error(`❌ ID=${id} NON TROUVÉ dans products.data.json`); return; }
  ...
  window.saveAddress = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        window.closeAccountPopup('address-popup');
      } else {
        window.showToast("Error: " + data.error);
      }
    } catch (err) {
      window.showToast("Network error while saving address");
    }
  };
  ...
  window.updatePassword = async () => {
    ...
    if (!email || !newPassword) return window.showToast("Email and new password are required");
    try {
      const res = await fetch('/.netlify/functions/save-account', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      const data = await res.json();
      if (data.success) { window.showToast("Password updated successfully!"); window.closeAccountPopup('password-popup'); }
      else window.showToast("Error: " + data.error);
    } catch (err) {
      window.showToast("Network error while updating password");
    }
  };
  ...
  window.trackOrder = () => {
    ...
    if (!num) { result.textContent = "Please enter a tracking number"; return; }
    result.textContent = `✅ Order ${num} tracked - Estimated arrival: 3-5 days`;
    setTimeout(() => window.closeAccountPopup('track-popup'), 4000);
  };
...
window.addEventListener('load', () => {
  ...
  if (isAccountPage && localStorage.getItem('isLoggedIn') !== 'true') {
    ...
    document.head.appendChild(hideStyle);
    setTimeout(() => {
      const overlayEl = document.getElementById('paulPopup');
  ...
  window.handleOrderItemClick = function(id) {
    ...
    if (url === 'shop.html') {
      if (typeof showErrorPopup === 'function') showErrorPopup(`Produit ID=${id} introuvable`);
      return;
    }
    window.location.href = url;
  };
