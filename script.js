...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (isAccountPage) {
    ...
    if (!isLoggedIn) {
      ...
      hideStyle.innerHTML = `body > *:not(#paulPopup) { display: none !important; } #paulPopup { display: flex !important; visibility: visible !important; opacity: 1 !important; z-index: 999999 !important; }`;
      document.head.appendChild(hideStyle);
      setTimeout(() => {
        openPaulPopup();
        const closeBtnPopup = document.querySelector('.paul-close');
        if (closeBtnPopup) { closeBtnPopup.style.pointerEvents = 'none'; closeBtnPopup.style.opacity = '0.3'; }
      }, 100);
      return;
    }
  ...
  async function loadAccountStats() {
    ...
    if (!email) return;
    try {
      const res  = await fetch('/.netlify/functions/save-account', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'get-stats', email }) });
      const data = await res.json();
  ...
  window.saveAddress = async () => {
    ...
    try {
      const res  = await fetch('/.netlify/functions/save-account', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'update-address', email, line1, line2, city, state, zip }) });
      const data = await res.json();
      if (data.success) { localStorage.setItem('userAddress', addressStr || 'No default address set'); document.getElementById('user-address').textContent = addressStr || 'No default address set'; window.showToast("Address saved successfully!"); window.closeAccountPopup('address-popup'); }
      else window.showToast("Error: " + data.error);
    } catch (err) { window.showToast("Network error while saving address"); }
  };
  ...
  window.updatePassword = async () => {
    ...
    try {
      const res  = await fetch('/.netlify/functions/save-account', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'update-password', email, newPassword }) });
      const data = await res.json();
      if (data.success) { window.showToast("Password updated successfully!"); window.closeAccountPopup('password-popup'); }
      else window.showToast("Error: " + data.error);
    } catch (err) { window.showToast("Network error while updating password"); }
  };
